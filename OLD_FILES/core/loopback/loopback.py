...
class LoopbackManager:
    ...
    async def refine_response(
        self, workflow_id: UUID, phase_id: UUID, response: str, max_iterations: int = 3
    ) -> Optional[str]:
        ...
        try:
            ...
            logger.info(
                f"🔄 Starting refinement for phase: {flow_context['phase_name']}"
            )

            current_response = response
            execution_id = await self._create_workflow_execution(
                workflow_id, flow_context["ai_model_id"]
            )

            for iteration in range(1, max_iterations + 1):
                try:
                    action_config = await self._get_refinement_action(
                        phase_id, iteration
                    )

                    if not action_config:
                        logger.info(
                            "⚠️ No more refinement actions configured. Stopping loopback."
                        )
                        break

                    refined_result, confidence_score = (
                        await self._execute_refinement_action(
                    ...
                    if refined_result and refined_result != current_response:
                        ...
                        logger.info(
                            f"✅ Response refined using action: {action_config['action_name']}"
                        )
                    else:
                        logger.info(
                            f"⚠️ No improvement from action: {action_config['action_name']}. Stopping loopback."
                        )
                        break
    ...
    async def _get_flow_context(
        self, workflow_id: UUID, phase_id: UUID
    ) -> Optional[Dict]:
    ...
    async def _get_refinement_action(
        self, phase_id: UUID, iteration: int
    ) -> Optional[Dict]:
    ...
    async def get_refinement_prompt(
        self, use_case_id: UUID, phase_id: UUID, model_id: UUID
    ) -> Optional[str]:
    ...
    async def _execute_refinement_action(
        ...
        try:
            prompt = await self.get_refinement_prompt(
                flow_context["use_case_id"],
                flow_context["phase_id"],
                action_config["model_id"],
            )
            logger.info(
                f"🔄 Attempting refinement with model: {action_config['model_name']}"
            )
            ...
            result = await batch_process_ai_inputs(
                enriched_prompt, action_config["model_name"]
            )

            # Fixed: Properly await the coroutine
            confidence_score = await self._evaluate_confidence(result)

            return (
                result.strip() if isinstance(result, str) else " ".join(result)
            ), confidence_score
    ...
    async def _create_workflow_execution(
        self, workflow_id: UUID, ai_model_id: UUID
    ) -> UUID:
    ...
    async def save_loopback_response(
        self,
        model_name: str,
        ...
        try:
            await conn.execute(
                ...
                INSERT INTO ai_responses (
                    model_name, response_text, workflow_execution_id, 
                    iteration, confidence_score, response_type
                )
                VALUES ($1, $2, $3, $4, $5, 'loopback_refinement');
            """,
                model_name,
                refined_response,
                execution_id,
            ...
            logger.info(f"✅ Saved loopback refinement for {model_name}")
        except Exception as e:
            logger.error(f"🚨 Error saving loopback response: {str(e)}")
        finally:
            await conn.close()
